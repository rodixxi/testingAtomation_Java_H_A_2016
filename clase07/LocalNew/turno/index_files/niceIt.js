...
(function($) {
	    ...
	    $(objForm).find('select').each(function (i, ctrl) {
	    	...
	    	nCtrl.find('li').each(function(num, elem) {
	    		...
	    		$(elem).bind('click', function() { 
	    			...
	    			$(ctrl).val($(this).attr('option')).change();
	...
	$.fn.fnReplaceFiles = function() {
		...
		$(objForm).find('input[type="file"]').each(function (i, ctrl) {
	    	...
	    	$(ctrl).before('<a tabindex="' + $(ctrl).attr('tabindex') + '" class="fnFileInput" id="' + id + '"><span><cite>Archivo ...</cite><strong>' + ($(ctrl).attr('title') != '' ? $(ctrl).attr('title') : 'Browse ...') + '</strong></span></a>');
	    	...
	    	/* Define Events  */
	    	$(ctrl).bind('change', function () { $(nCtrl).find('cite').text($(this).val()); });
